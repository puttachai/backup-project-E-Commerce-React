...
const Pagination = ({ currentItems = [], selectedBrands = [], selectedCategories = [] }) => {
  ...
  return (
    <>
      {filteredPagination.map((pagination) => (
        <div key={pagination.id} className="w-full">
          ...
          <Product
            ...
            price={pagination.price}
            qty={pagination.qty}
            img={`${BASE_URL}/images/product/${pagination.image}`} // ใช้ฟิลด์ image2 จากฐานข้อมูล http://localhost:5000
...
const Paginations = ({ itemsPerPage }) => {
  ...
  useEffect(() => {
    axios.get(`${BASE_URL}/api/Pagination_laravel`, { //http://localhost:5000
      method: 'GET',
      headers: {
